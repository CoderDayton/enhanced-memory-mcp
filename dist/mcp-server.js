...
server.registerTool('auto_tag', {
    ...
    switch (operation) {
        case 'analyze':
            if (!content)
                throw new Error('Content required for analyze operation');
        ...
        case 'apply':
            if (!memoryId)
                throw new Error('memoryId required for apply operation');
        ...
        case 'preview':
            if (!memoryId)
                throw new Error('memoryId required for preview operation');
...
server.registerTool('similarity', {
    ...
    switch (operation) {
        case 'find_similar':
            if (!content)
                throw new Error('Content required for find_similar operation');
...
server.registerTool('batch', {
    ...
    switch (operation) {
        case 'delete_by_type':
            if (!type)
                throw new Error('Type required for delete_by_type operation');
            result = await memoryStore.deleteByType(type, confirm);
            break;
        case 'delete_by_tags':
            if (!tags)
                throw new Error('Tags required for delete_by_tags operation');
            result = await memoryStore.deleteByTags(tags, confirm);
            break;
        case 'update_by_type':
            if (!type || !updates)
...
server.registerTool('workflow', {
    ...
    switch (operation) {
        case 'auto_tag':
            ...
            const recentMemories = await memoryStore.getRecentMemories(opts.maxActions || 100, opts.timeframe || 'day');
            let taggedCount = 0;
            ...
            for (const memory of recentMemories.slice(0, opts.maxActions || 10)) {
                try {
                    ...
                    const suggestedTags = await memoryStore.generateAutoTags(memory.content, opts.maxTagsPerMemory || 5);
